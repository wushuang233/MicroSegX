...
Helm chart for MicroSegX container security's core services.

## CRD
Because the CRD (Custom Resource Definition) policies can be deployed before MicroSegX's core product, a new 'crd' helm chart is created. The crd template in the 'core' chart is kept for the backward compatibility. Please set `crdwebhook.enabled` to false, if you use the new 'crd' chart.
...
`registry` | MicroSegX container registry | `docker.io` |
`tag` | image tag for controller enforcer manager | `latest` |
`oem` | OEM release name | `nil` |
`imagePullSecrets` | image pull secret | `nil` |
`rbac` | MicroSegX RBAC Manifests are installed when RBAC is enabled | `true` | Required for Rancher Authentication. |
`psp` | MicroSegX Pod Security Policy when psp policy is enabled | `false` |
`serviceAccount` | Service account name for MicroSegX components | `default` |
...
`internal.certmanager.secretname` | Name of the secret to be used for the internal certificates | `microsegx-internal` |
...
`global.aws.enabled` | If true, install AWS billing csp adapter | `false` | **Note**: default admin user is disabled when aws market place billing enabled, use secret to create admin-role user to manage MicroSegX deployment.
...
`global.aws.image.repository` | csp adapter image repository | `microsegx/microsegx-csp-adapter` | Follow AWS subscription instruction
...
`global.azure.enabled` | If true, install Azure billing csp adapter | `false` | **Note**: default admin user is disabled when azure market place billing enabled, use secret to create admin-role user to manage MicroSegX deployment.
...
`global.azure.images.microsegx_csp_pod.registry` | csp adapter image registry | `susellcforazuremarketplace.azurecr.io` | Follow Azure subscription instruction
`global.azure.images.microsegx_csp_pod.image` | csp adapter image repository | `microsegx-billing-azure-by-suse-llc` | Follow Azure subscription instruction
`global.azure.images.microsegx_csp_pod.digest` | csp adapter image digest | `nil` | Follow Azure subscription instruction
`global.azure.images.microsegx_csp_pod.imagePullPolicy` | csp adapter image pull policy | `IfNotPresent` | Follow Azure subscription instruction
...
`controller.configmap.enabled` | If true, configure MicroSegX global settings using a ConfigMap | `false`
`controller.configmap.data` | MicroSegX configuration in YAML format | `{}`
`controller.secret.enabled` | If true, configure MicroSegX global settings using secrets | `false`
`controller.secret.data` | MicroSegX configuration in key/value pair format | `{}`
...
`cve.adapter.image.repository` | registry adapter image repository | `microsegx/registry-adapter` |
...
`cve.updater.image.repository` | cve updater image repository | `microsegx/updater` |
...
`cve.scanner.image.repository` | cve scanner image repository | `microsegx/scanner` |
...
```console
$ helm install my-release --namespace microsegx ./microsegx-helm/ --set manager.env.ssl=off
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart. For example,

```console
$ helm install my-release --namespace microsegx ./microsegx-helm/ -f values.yaml
```
