...
patch_runtime_port_audit_address() {
  ...
  if [[ -z "${base_url}" ]]; then
    host_port=$(resolve_port_audit_host_port "${port_audit_namespace}" "k8s-port-audit" || true)
    node_ip=$(resolve_first_node_internal_ip)
    if [[ -n "${host_port}" && -n "${node_ip}" ]]; then
      base_url="http://${node_ip}:${host_port}"
    else
      service_ip=$(wait_for_service_ip "${port_audit_namespace}" "k8s-port-audit" 90 || true)
      if [[ -n "${service_ip}" ]]; then
        base_url="http://${service_ip}:8080"
      fi
    fi
  fi

  if [[ -z "${base_url}" ]]; then
    echo "==> Skipping runtime port-audit address patch: service IP not ready"
    return 0
  fi
