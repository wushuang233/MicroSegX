...
func TestIngressController(t *testing.T) {
	...
	for i, output := range outs {
		...
		switch i {
		case 0:
			if ing.Name != "microsegx-restapi-ingress" {
				t.Errorf("Ingress name is wrong. name=%v\n", ing.Name)
			}
		case 1:
			if ing.Name != "microsegx-mastersvc-ingress" {
				t.Errorf("Ingress name is wrong. name=%v\n", ing.Name)
			}
		case 2:
			if ing.Name != "microsegx-managedsvc-ingress" {
				t.Errorf("Ingress name is wrong. name=%v\n", ing.Name)
			}
		}
	}
...
func TestIngressManager(t *testing.T) {
	...
	for i, output := range outs {
		...
		switch i {
		case 0:
			if ing.Name != "microsegx-webui-ingress" {
				t.Errorf("Ingress name is wrong. name=%v\n", ing.Name)
			}
		}
	}
