...
func checkControllerServiceDefault(t *testing.T, svc corev1.Service) {
	if svc.Name != "microsegx-svc-controller" {
...
func checkControllerServiceAPI(t *testing.T, svc corev1.Service, svcType string) {
	if svc.Name != "microsegx-svc-controller-api" {
		t.Errorf("Service name is wrong. name=%v\n", svc.Name)
	}
...
func checkControllerServiceFedMaster(t *testing.T, svc corev1.Service, svcType string) {
	if svc.Name != "microsegx-svc-controller-fed-master" {
		t.Errorf("Service name is wrong. name=%v\n", svc.Name)
	}
	if string(svc.Spec.Type) != svcType {
		t.Errorf("Service type is wrong. type=%v\n", svc.Spec.Type)
...
func checkControllerServiceFedManaged(t *testing.T, svc corev1.Service, svcType string) {
	if svc.Name != "microsegx-svc-controller-fed-managed" {
		t.Errorf("Service name is wrong. name=%v\n", svc.Name)
	}
	if string(svc.Spec.Type) != svcType {
		t.Errorf("Service type is wrong. type=%v\n", svc.Spec.Type)
...
func checkManagerService(t *testing.T, svc corev1.Service, svcType string) {
	if svc.Name != "microsegx-service-webui" {
		t.Errorf("Service name is wrong. name=%v\n", svc.Name)
	}
	if string(svc.Spec.Type) != svcType {
		t.Errorf("Service type is wrong. type=%v\n", svc.Spec.Type)
	...
	if app, ok := svc.Spec.Selector["app"]; !ok || app != "microsegx-manager-pod" {
		t.Errorf("Service selector is invalid. selector=%+v\n", svc.Spec.Selector)
	}
}
...
func TestFedMasterServiceLBIP(t *testing.T) {
	helmChartPath := "../charts/core"

	svcType := "LoadBalancer"
	options := &helm.Options{
		SetValues: map[string]string{
			"controller.federation.mastersvc.type":           svcType,
			"controller.federation.mastersvc.loadBalancerIP": "1.2.3.4",
		},
	}
	...
	checkControllerServiceFedMaster(t, svc, svcType)
}

func TestFedManagedServiceLBIP(t *testing.T) {
	helmChartPath := "../charts/core"

	svcType := "LoadBalancer"
	options := &helm.Options{
		SetValues: map[string]string{
			"controller.federation.managedsvc.type":           svcType,
			"controller.federation.managedsvc.loadBalancerIP": "1.2.3.4",
		},
	}
