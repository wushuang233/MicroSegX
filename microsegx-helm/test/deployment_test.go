...
func TestControllerDeploymentRegistry(t *testing.T) {
	...
	options := &helm.Options{
		SetValues: map[string]string{
			"registry":         "registry.microsegx.com",
			"autoGenerateCert": "false",
		},
	}
	...
	var dep appsv1.Deployment
	helm.UnmarshalK8SYaml(t, outs[0], &dep)
	if strings.HasPrefix(dep.Spec.Template.Spec.Containers[0].Image, "registry.microsegx.com/controler:") {
...
func TestControllerDeploymentOEM(t *testing.T) {
	...
	var dep appsv1.Deployment
	helm.UnmarshalK8SYaml(t, outs[0], &dep)
	if dep.Spec.Template.Spec.Containers[0].Image != "registry.microsegx.com/oem/controller:0.9" {
		t.Errorf("Image location is wrong, %v\n", dep.Spec.Template.Spec.Containers[0].Image)
	}
}
...
func checkManagerDeployment(t *testing.T, dep appsv1.Deployment, ssl bool) {
	if dep.Name != "microsegx-manager-pod" {
...
func TestControllerSecrets(t *testing.T) {
	...
	for _, output := range outs {
		...
		if secret.Name == "microsegx-controller-secret" {
	...
	for _, output := range outs {
		var dep appsv1.Deployment
		helm.UnmarshalK8SYaml(t, output, &dep)
		if dep.Name == "microsegx-controller-pod" {

			assert.Contains(t, dep.Spec.Template.Spec.Volumes, corev1.Volume{
				Name: "cert",
				VolumeSource: corev1.VolumeSource{
					Secret: &corev1.SecretVolumeSource{
						SecretName: "microsegx-controller-secret",
					},
				},
			})
		}
	...
	for _, output := range outs {
		...
		if dep.Name == "microsegx-controller-pod" {
			...
			for _, container := range dep.Spec.Template.Spec.Containers {
				if container.Name == "microsegx-controller-pod" {

					assert.Contains(t, container.VolumeMounts, corev1.VolumeMount{
						Name:      "cert",
						MountPath: "/etc/microsegx/certs/ssl-cert.key",
					...
					assert.Contains(t, container.VolumeMounts, corev1.VolumeMount{
						Name:      "cert",
						MountPath: "/etc/microsegx/certs/ssl-cert.pem",
...
func TestControllerNoSecrets(t *testing.T) {
	...
	for _, output := range outs {
		var secret corev1.Secret
		helm.UnmarshalK8SYaml(t, output, &secret)
		assert.NotEqual(t, "microsegx-controller-secret", secret.Name)
	}
