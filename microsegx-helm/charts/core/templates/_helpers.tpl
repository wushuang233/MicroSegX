...
Lookup secret.
*/}}
{{- define "microsegx.secrets.lookup" -}}
{{- $value := "" -}}
...
{{- else if .Values.controller.image.hash }}
  ...
  {{- else }}
    {{- printf "%s@%s" .Values.controller.image.repository .Values.controller.image.hash }}
  {{- end }}
{{- else }}
  {{- if .Values.registry }}
