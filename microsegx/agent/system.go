...
func cbGetLearnedGroupName(id string) (string, bool, bool) {
	svc, _, bMicrosegx := getContainerService(id)
	switch svc {
	case "":
	...
	return makeLearnedGroupName(utils.NormalizeForURL(svc)), true, bMicrosegx
}
...
func processPolicyLookup(id, riskType, pname, ppath string, pid, pgid, shellCmd int, ppe *share.CLUSProcessProfileEntry) (string, string, string, string, bool, error) {
	...
	svc, capBlock, bMicrosegx := getContainerService(id)
	switch svc {
	case "":
	...
	if err == nil {
		...
		case share.PolicyActionViolate: // policy mode decision
			if !bMicrosegx && pe.IsAllowedByParentApp(svcGroup, id, ppe.Name, pname, ppath, pgid) {
