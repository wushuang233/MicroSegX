...
func gInfoReadMicrosegxContainer(id string) (*containerData, bool) {
	gInfoRLock()
...
func nvPod2Role(pod string) string {
	// Assume the pod name must be of pattern, microsegx-controller-pod-xxxx or dss-controller-pod-xxxx (OEM)
	if !strings.HasPrefix(pod, "microsegx-") && !strings.HasPrefix(pod, "dss-") {
		return ""
	}
...
func isMicrosegxFunctionRole(role string, rootPid int) bool {
...
func isMicroSegXContainer(info *container.ContainerMetaExtra) (string, bool) {
	labels := info.Labels
	if Agent.Domain != "" && Agent.Domain != global.ORCH.GetDomain(labels) { // orchestra
		return "", false
	}

	if role, ok := labels[share.MicroSegXLabelRole]; ok {
		if isMicrosegxFunctionRole(role, info.Pid) {
			return role, true
		}
	}
...
func getMicroSegXRole(info *container.ContainerMetaExtra) (string, bool) {
	labels := info.Labels
	if Agent.Domain != "" && Agent.Domain != global.ORCH.GetDomain(labels) { // orchestra
		return "", false
	}

	role, ok := labels[share.MicroSegXLabelRole]
	return role, ok
}
...
func taskReexamIntfContainer(id string, info *container.ContainerMetaExtra, restartMonitor bool) {
	if c, ok := gInfoReadMicrosegxContainer(id); ok {
		if info != nil && c.pid != info.Pid {
			return
		}

		examMicroSegXInterface(c, changeIntf)
		return
	}
...
// ////
func examMicroSegXInterface(c *containerData, change int) {
...
func isMicrosegxContainerById(id string) (string, bool) {
	if c, ok := gInfoReadMicrosegxContainer(id); ok {
...
// ////
func startMicroSegXMonitors(id, role string, info *container.ContainerMetaExtra) {
	...
	if _, ok := isMicrosegxContainerById(id); ok { // existed and ignore it
		return
	}
	...
	c := &containerData{
		...
		pid:            info.Pid,
		info:           info,
		role:           container.PlatformContainerMicroSegX,
	...
	// Because activePid2ID doesn't have microsegx container in it, we cannot always get
	...
	//          like controller =>  "microsegx (Domain/Namespace) + microsegx-controller-pod (Name)""
	svc := global.ORCH.GetService(&info.ContainerMeta, Host.Name)
	c.service = utils.MakeServiceName(svc.Domain, svc.Name)
	c.domain = svc.Domain
	...
	if !isChild {
		if c.pid != 0 {
			...
			examMicroSegXInterface(c, changeInit)
		}
	} else {
	...
	// process monitor : protect mode, process profiles for all microsegx containers
	if agentEnv.containerShieldMode {
		prober.BuildProcessFamilyGroups(c.id, c.nvRole, c.pid, false, info.Privileged, nil)
		pe.InsertMicrosegxProcessProfilePolicy(group, role)
		c.upperDir, c.rootFs, _ = lookupContainerLayerPath(c.pid, c.id)
...
func stopMicroSegXMonitor(c *containerData) {
	...
	// and send both STOP and DEL event, because we only care when MicroSegX container is running.
...
func taskAddContainer(id string, info *container.ContainerMetaExtra) {
	...
	if role, ok := isMicroSegXContainer(info); ok {
		if info.Running {
			startMicroSegXMonitors(id, role, info)
		} else {
			if info.Pid != 0 {
				log.WithFields(log.Fields{"id": id, "role": role, "pid": info.Pid}).Debug("PROC: exited MicroSegX")
			}
			// Sending notification to controller for MicroSegX containers is to report
...
func intfHostMonitorLoop(hid string, stopCh chan struct{}) {
	...
	for {
		...
		case updateLink := <-chLink:
			if updateLink.Link == nil || updateLink.Attrs() == nil {
				continue
			}
