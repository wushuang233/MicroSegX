...
var (
	ControllerPodLabelSelector = fields.OneTermEqualSelector("app", "microsegx-controller-pod").String()
	ScannerPodLabelSelector    = fields.OneTermEqualSelector("app", "microsegx-scanner-pod").String()
	EnforcerPodLabelSelector   = fields.OneTermEqualSelector("app", "microsegx-enforcer-pod").String()
	RegistryPodLabelSelector   = fields.OneTermEqualSelector("app", "microsegx-registry-adapter-pod").String()
)
...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:    "pod-namespace",
			Value:   "microsegx",
			Usage:   "The k8s namespace where MicroSegX is running in",
			EnvVars: []string{"POD_NAMESPACE"},
		},
		&cli.StringFlag{
			Name:    "internal-secret-name",
			Value:   "microsegx-internal-certs",
	...
	app.Commands = cli.Commands{
		&cli.Command{
			...
			Usage:  "This command creates upgrader job from microsegx-upgrader-pod cron job in a not racey way.",
			Flags:  []cli.Flag{},
			Action: PreSyncHook,
		},
		&cli.Command{
			Name:  "upgrader-job",
			Usage: "Run microsegx upgrader",
			Flags: []cli.Flag{
				&cli.DurationFlag{
