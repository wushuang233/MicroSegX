...
func TestIsFreshInstall(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		isFreshInstall, err := IsFreshInstall(ctx.Context, client, "microsegx")
		if tc.ExpectedErr {
			assert.NotNil(t, err)
		} else {
			assert.Nil(t, err)
...
func TestCreateJob(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		job, err := CreatePostSyncJob(ctx.Context, client, "microsegx", "uid", false)
...
func TestCreateJobWithForcedK8sError(t *testing.T) {
	...
	ErrorInjector(t, nil, func(ctx *cli.Context, client dynamic.Interface, injected *bool) bool {
		job, err := CreatePostSyncJob(ctx.Context, client, "microsegx", "uid2", false)
