...
type RESTTokenRedirect struct {
	// The MicroSegX URL to redirect after authentication/logout.
	Redirect string `json:"redirect_endpoint"`
	// (Optional)
...
type RESTEULAData struct {
	...
	K8sRbacAlertMsg string    `json:"k8s_rbac_alert_message,omitempty"` // alert message when k8s role/rolebinding microsegx-binding-secret is not correct
}
...
type RESTSysNetConfigConfig struct {
	...
	StrictGroupMode      *bool   `json:"strict_group_mode,omitempty"`
}
...
type RESTSystemConfig struct {
	...
	EnableTLSVerification      bool                      `json:"enable_tls_verification"`
	GlobalCaCerts              []string                  `json:"cacerts"`
	AllowNsUserExportNetPolicy bool                      `json:"allow_ns_user_export_net_policy"`
}
