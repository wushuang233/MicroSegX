...
func (s *Syslogger) makeDial(prio syslog.Priority) (*syslog.Writer, error) {
	if s.proto == "tcp+tls" {
		return syslog.DialWithTLSCert("tcp+tls", s.addr, syslogFacility|prio, "microsegx", []byte(s.serverCert))
	}

	return syslog.Dial(s.proto, s.addr, syslogFacility|prio, "microsegx")
}

// --

// const webhookInfo = "Microsegx webhook is configured."
...
func (w *Webhook) Notify(elog interface{}, level, category, cluster, title, comment string, proxy *share.CLUSProxy) {
	...
	if logText := struct2Text(elog); logText != "" {
		...
		case api.WebhookTypeSlack:
			...
			fields["text"] = logText
			fields["username"] = fmt.Sprintf("MicroSegX - %s", cluster)
		...
		case api.WebhookTypeJSON:
			...
			var extra string
			if comment != "" {
				extra = fmt.Sprintf("{\"cluster\":\"%s\",\"comment\":\"%s\",", cluster, comment)
			} else {
