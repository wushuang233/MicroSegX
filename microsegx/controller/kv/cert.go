...
const (
	AdmCAKeyPath  = "/etc/microsegx/certs/internal/adm_ca.key"
	AdmCACertPath = "/etc/microsegx/certs/internal/adm_ca.cert"
...
func CreateCAFilesAndStoreInKv(certpath, keypath string) error {
	...
	if !notfound {
		...
		// In fresh deployment, customer maps a folder to controller pod's /etc/microsegx/certs/internal/ and provide their own root CA.
...
func GetDefaultCACertTemplate() *x509.Certificate {
	return &x509.Certificate{
		...
		Subject: pkix.Name{
			Country:            []string{"US"},
			Organization:       []string{"Microsegx"},
			OrganizationalUnit: []string{"Microsegx"},
			Locality:           []string{"EN"},
			Province:           []string{"CA"},
		},
...
func verifyWebServerCert(cn string, certData []byte) bool {
	...
	if len(certData) > 0 {
		var customerCert bool // true means the cert is generated by customer, not microsegx
		var err error
		if block, _ := pem.Decode(certData); block == nil {
			...
			if e != nil {
				...
				if len(subj.Country) == 1 && len(subj.Organization) == 1 && len(subj.OrganizationalUnit) == 1 && len(subj.Province) == 1 &&
					subj.Country[0] == "US" && subj.Organization[0] == "Microsegx" && subj.OrganizationalUnit[0] == "Microsegx" && subj.Province[0] == "CA" {
					// this cert is generated by microsegx
				} else {
					customerCert = true
				}
...
func GetFedTlsKeyCertPath(masterID, jointID string) (string, string, string) { // returns (caCertPath, privKeyPath, certPath)
	...
	if masterID != "" {
		caCertPath = fmt.Sprintf("/etc/microsegx/certs/fed.master.%s.cert.pem", masterID)
	}
	if jointID != "" {
		privKeyPath = fmt.Sprintf("/etc/microsegx/certs/fed.client.%s.key.pem", jointID)
		certPath = fmt.Sprintf("/etc/microsegx/certs/fed.client.%s.cert.pem", jointID)
	}
