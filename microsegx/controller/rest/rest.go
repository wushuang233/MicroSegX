...
import (
	...
	"github.com/wushuang233/MicroSegX/microsegx/controller/remote_repository"
...
var _teleMicrosegxURL string
var _teleFreq uint

const defaultSSLCertFile = "/etc/microsegx/certs/ssl-cert.pem"
const defaultSSLKeyFile = "/etc/microsegx/certs/ssl-cert.key"

const defFedSSLCertFile = "/etc/microsegx/certs/fed-ssl-cert.pem"
const defFedSSLKeyFile = "/etc/microsegx/certs/fed-ssl-cert.key"
...
var restErrMessage = []string{
	...
	api.RESTErrK8sNvRBAC:             "Kubernetes RBAC settings required for MicroSegX is not configured correctly",
	api.RESTErrWebhookSvcForAdmCtrl:  "The microsegx-svc-admission-webhook service required for MicroSegX Admission Control is not configured correctly",
	api.RESTErrNoUpdatePermission:    "MicroSegX controller doesn't have UPDATE permission for service resource",
	api.RESTErrK8sApiSrvToWebhook:    "Failed to receive a request from Kube-apiserver. Please try different client mode",
	api.RESTErrNvPermission:          "MicroSegX controller is forbidden to get service details. Please check the clusterrole/clusterrolebinding required for MicroSegX default service account",
	api.RESTErrWebhookIsDisabled:     "Configuring MicroSegX Admission Control global settings is not allowed when admission control is disabled",
...
func InitContext(ctx *Context) {
	...
	_teleMicrosegxURL = ctx.TeleMicrosegxURL
...
func StartStopFedPingPoll(cmd, interval uint32, param1 interface{}) error {
	...
	case share.ReportTelemetryData:
		if param1 != nil && _teleMicrosegxURL != "" {
