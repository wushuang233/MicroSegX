...
const (
	tlsClientCA = "/var/microsegx/clientCA.cert.pem"
)

// const (
// 	admissionWebhookAnnotationStatusKey = "microsegx-mutating-admission-webhook/status"
// 	admissionWebhookLabelKey            = "microsegx-mutating-admission-webhook/label"
// )
			...
			case OPERATION_UPDATE:
				...
				if err := json.Unmarshal(req.Object.Raw, &svc); err == nil && svc.Labels != nil {
					...
					if tag, ok := svc.Labels[tagKey]; ok && tag != "" {
						// if label 'echo-microsegx-svc-admission-webhook' has the same value as label 'tag-microsegx-svc-admission-webhook',
