...
func flushEventQueue() {
	...
	if err := cacher.FlushAdmCtrlStats(); err != nil {
		log.WithFields(log.Fields{"error": err}).Debug("FlushAdmCtrlStats")
	}
}
...
func main() {
	...
	teleMicrosegxEP := flag.String("telemetry_microsegx_ep", "", "")                   // for testing only
	...
	maxConcurrentRepoScanTasks := flag.Int("max_concurrent_repo_scan_tasks", 6, "Maximum number of concurrent repository scan tasks")
	...
	if Ctrler.Leader {
		...
		if isNewCluster && *noDefAdmin {
			if err := clusHelper.DeleteUser(common.DefaultAdminUser); err != nil {
				log.WithFields(log.Fields{"error": err}).Fatal("Failed to delete default admin user")
			}
		}
		setConfigLoaded()
	} else {
	...
	if strings.HasSuffix(*teleMicrosegxEP, "apikeytest") {
	...
	if value, _ := cluster.Get(share.CLUSCtrlVerKey); value != nil {
		...
		if !strings.HasPrefix(ver.CtrlVersion, "interim/") {
			// it's official release image
			if *teleMicrosegxEP == "" {
				// use public microsegx-upgrade-responder if testing upgrade-responder url is not specified
				*teleMicrosegxEP = "https://upgrades.microsegx-upgrade-responder.livestock.rancher.io/v1/checkupgrade"
			}
		}
	}
	if *teleMicrosegxEP == "" {
	...
	rctx := rest.Context{
		...
		TeleMicrosegxURL:        *teleMicrosegxEP,
	...
	go rest.StartRESTServer(
		isNewCluster,
		Ctrler.Leader,
		*maxConcurrentRepoScanTasks, // Max concurrent repository scan
		*scanJobQueueCapacity,       // Capacity of scan job queue
		*scanJobFailRetryMax,        // Max retries for failed scan jobs
		time.Duration(*repoScanLongPollTimeout)*time.Second,       // Long poll timeout duration
		time.Duration(*staleScanJobCleanupIntervalHour)*time.Hour, // Cleanup interval for stale jobs
	)
				...
				if o, err := global.ORCH.GetResource(resource.RscTypePod, domain.Name, podname); err == nil {
					if pod := o.(*resource.Pod); pod != nil {
						...
						if Ctrler.Labels != nil {
							...
							Ctrler.Labels["name"] = share.MicroSegXRoleController
							Ctrler.Labels["microsegx.role"] = share.MicroSegXRoleController
							Ctrler.Labels["release"] = ""
							Ctrler.Labels["vendor"] = "MicroSegX Inc."
