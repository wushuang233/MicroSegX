...
const (
	customRulesConfigmap = "microsegx-custom-rules"
		...
		for {
			...
			case ev := <-cctx.OrchChan:
				...
				case resource.RscTypePod:
					...
					if n != nil {
						if o == nil { // create
							if !isMicrosegxContainerName(n.Name) {
				...
				case resource.RscTypeDeployment:
					...
					if n != nil {
						if n.Domain == resource.NvAdmSvcNamespace && n.Name == "microsegx-scanner-pod" {
							atomic.StoreUint32(&scannerReplicas, uint32(n.Replicas))
						}

						if o == nil && n.Name == "microsegx-csp-pod" {
							log.WithFields(log.Fields{"name": n.Name, "domain": n.Domain}).Info("detected")
						}
					} else if o != nil { // delete
						if o.Domain == resource.NvAdmSvcNamespace && o.Name == "microsegx-scanner-pod" {
							atomic.StoreUint32(&scannerReplicas, 0)
						}

						if o.Name == "microsegx-csp-pod" {
	...
	if time.Now().After(nextKeyRotationTime) {
		if err := resource.AddStorePassphrase(); err == nil {
			if err = kv.SetNextKeyRotationTime(cctx.KeyRotationDuration); err == nil {
				alert := "Important: Please backup the data in Kubernetes secret microsegx-store-secret reset by MicroSegX"
