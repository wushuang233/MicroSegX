...
var whRevertCount uint32   // ValidatingWebhookConfiguration microsegx-validating-admission-webhook revert count (because of unknown matchExpressions keys)
...
func updateNvDeployStatus(status map[string]bool) (updated, nvInstalled bool) {
	...
	if len(status) > 0 {
		for k, v := range nvDeployStatus {
			...
			// (microsegx-controller-pod/microsegx-allinone-pod & microsegx-svc-admission-webhook) are all gone,
...
func isAdmissionRuleMet(admResObject *nvsysadmission.AdmResObject, c *nvsysadmission.AdmContainerInfo, scannedImage *nvsysadmission.ScannedImageSummary,
	...
	if hasCustomCriteria {
		...
		policyUrl := fmt.Sprintf("/v1/data/microsegx_policy_%d", ruleID)
...
func (m CacheMethod) SetNvDeployStatusInCluster(resName string, value bool) {
	...
	// resName, like "microsegx-controller-pod"/"microsegx-svc-admission-webhook"/"microsegx-svc-crd-webhook", is used as key in status map
	retry := 0
	for retry < retryClusterMax {
