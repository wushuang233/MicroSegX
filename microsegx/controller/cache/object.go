...
func addrDeviceAdd(id string, ifaces map[string][]share.CLUSIPAddr) {
	...
	//than agent get updated which cause microsegx device temprary to be unmanaged
	//workload, reset unManagedWlTimer to update unmanaged workload ip to prevent
	//microsegx device being treated as unmanaged wl
	if addcnt > 0 {
...
func workloadUpdate(nType cluster.ClusterNotifyType, key string, value []byte) {
	...
	case cluster.ClusterNotifyAdd, cluster.ClusterNotifyModify:
		...
		// Check if it's MicroSegX containers first
		if wl.PlatformRole == container.PlatformContainerMicroSegX {
