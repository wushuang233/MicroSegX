...
func isMicrosegxContainerGroup(group string) bool {
	if dot := strings.LastIndex(group, "."); dot != -1 {
		if group[dot+1:] == localDev.Ctrler.Domain {
			name := group[:dot]
			nvContainers := map[string]interface{}{
				"nv.microsegx-manager-pod":          nil,
				"nv.microsegx-scanner-pod":          nil,
				"nv.microsegx-controller-pod":       nil,
				"nv.microsegx-enforcer-pod":         nil,
				"nv.microsegx-updater-pod":          nil,
				"nv.microsegx-csp-pod":              nil,
				"nv.microsegx-registry-adapter-pod": nil,
				"nv.microsegx-cert-upgrader-pod":    nil,
			}
...
func groupConfigUpdate(nType cluster.ClusterNotifyType, key string, value []byte) {
	...
	case cluster.ClusterNotifyAdd, cluster.ClusterNotifyModify:
		...
		if isMicrosegxContainerGroup(group.Name) {
			...
			log.WithFields(log.Fields{"group": group.Name}).Info("Delete microsegx group")
			return
		}
...
func createServiceIPGroup(r *resource.Service) *share.CLUSGroup {
	...
	// Ignore the service with label key contains 'microsegx'. This is probably from docker swarm
	for k, v := range r.Labels {
		if strings.Contains(strings.ToLower(k), "microsegx") {
			log.WithFields(log.Fields{"key": k, "value": v}).Debug("Ignore service with microsegx label")
			return nil
		}
	}
...
func isMicrosegxContainerName(name string) bool {
	if matched, err := regexp.MatchString(`^microsegx-(controller|enforcer|manager|allinone|updater|scanner|csp|registry-adapter)-pod`, name); err == nil {
		return matched
	}
	return false
}
