...
func (m CacheMethod) GetSystemConfig(acc *access.AccessControl) *api.RESTSystemConfig {
	...
	rconf := api.RESTSystemConfig{
		...
		AllowNsUserExportNetPolicy: systemConfigCache.AllowNsUserExportNetPolicy,
	}
...
func systemConfigUpdate(nType cluster.ClusterNotifyType, key string, value []byte) {
	...
	case cluster.ClusterNotifyAdd, cluster.ClusterNotifyModify:
		...
		go func() {
			...
			defer cancel()
			err := rpc.ScannerAcquisitionMgr.RunTaskForEachScanner(func(client share.ScannerServiceClient) error {
