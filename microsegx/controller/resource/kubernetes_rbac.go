...
type k8sRbacRoleInfo struct { // for role & cluster role
	...
	namespace     string    // "{nv}" means microsegx namespace. "" means cluster role
...
// ------------------------------------------------------------------------------------------------
// fedAdmin:    in {"read-only.microsegx.api.io", "*"}	 	"*"				in {"*"}    // "cattle-globalrole-....." clusterrole(with clusterrolebinding)
// fedReader:   in {"read-only.microsegx.api.io", "*"}	 	"*"				in {"get}   // "cattle-globalrole-....." clusterrole(with clusterrolebinding)
// admin:       in {"read-only.microsegx.api.io", "*"}	 	"*"				in {"*"}    // clusterrole(with clusterrolebinding)
// reader:      in {"read-only.microsegx.api.io", "*"}		"*"				in {"get"}  // clusterrole(with clusterrolebinding)
// ns admin:    in {"read-only.microsegx.api.io", "*"}		"*"				in {"*"}    // clusterrole(with rolebinding) or role
// ns reader:   in {"read-only.microsegx.api.io", "*"}		"*"				in {"get"}  // clusterrole(with rolebinding) or role
...
var rbacRolesWanted map[string]*k8sRbacRoleInfo = map[string]*k8sRbacRoleInfo{ // (cluster) role settings required by nv
	...
	nvCrdGrpDefRole: {
		name: nvCrdGrpDefRole,
		rules: []*k8sRbacRoleRuleInfo{
			{
				apiGroup:  constApiGroupNV,
				resources: utils.NewSet(RscTypeCrdGroupDefinition),
				verbs:     crdPolicyRoleVerbs,
			},
		},
	},
	...
	if event == WatchEventDelete {
		...
		if roleInfo, ok := rbacRolesWanted[o.name]; ok && !roleInfo.k8sReserved {
			...
			if o.name == NvScannerRole {
				if _, err := global.ORCH.GetResource(RscTypeCronJob, NvAdmSvcNamespace, "microsegx-updater-pod"); err != nil {
					evLog = false
				} else if errs, _ := VerifyNvRbacRoleBindings([]string{NvAdminRoleBinding}, false, false); len(errs) == 0 {
					// when microsegx-admin rolebinding is configured correctly, do not report event about role microsegx-binding-scanner being deleted
					evLog = false
				}
			}
			if evLog {
		...
		/*
			// in case the clusterrole microsegx-binding-customresourcedefinition is recreated/updated(after nv 5.0 deployment)
		...
		if roleInfo, ok := rbacRolesWanted[n.name]; ok && !roleInfo.k8sReserved {
			...
			if n.name == NvScannerRole {
				if _, err := global.ORCH.GetResource(RscTypeCronJob, NvAdmSvcNamespace, "microsegx-updater-pod"); err != nil {
					...
					// when microsegx-admin rolebinding is configured correctly, do not care role microsegx-binding-scanner's settings
					checkRBAC = false
				}
			}
			if checkRBAC {
		...
		if bindingInfo, ok := rbacRoleBindingsWanted[o.name]; ok && o.domain == bindingInfo.namespace {
			...
			if o.name == NvAdminRoleBinding || o.name == NvScannerRoleBinding {
				if _, err := global.ORCH.GetResource(RscTypeCronJob, NvAdmSvcNamespace, "microsegx-updater-pod"); err != nil {
					...
					// when one of (microsegx-admin, microsegx-binding-scanner) rolebinding is configured correctly, do not care another rolebinding is deleted
		...
		// 5. nv-required clusterrolebinding check
		{
			if bindingInfo, ok := rbacRoleBindingsWanted[n.name]; ok && n.domain == bindingInfo.namespace {
				...
				if n.name == NvAdminRoleBinding || n.name == NvScannerRoleBinding {
					if _, err := global.ORCH.GetResource(RscTypeCronJob, NvAdmSvcNamespace, "microsegx-updater-pod"); err == nil {
						// rolebinding microsegx-binding-scanner is preferred in 5.2(+)
						// rolebinding microsegx-admin is majorly for backward compatibility
						if errs, _ = VerifyNvRbacRoleBindings([]string{NvAdminRoleBinding}, false, false); len(errs) > 0 {
							// access denied for reading rolebinding resources, rolebinding microsegx-admin is not found or it's incorrectly configured
							if errs2, k8sRbac403 := VerifyNvRbacRoleBindings([]string{NvScannerRoleBinding}, false, true); !k8sRbac403 && len(errs) > 0 {
								// rolebinding microsegx-binding-scanner is not found or it's incorrectly configured
								errs = errs2
							}
						}
					}
...
func VerifyNvK8sRBAC(flavor, csp string, existOnly bool) ([]string, []string, []string, []string) {
	...
	// check microsegx-updater-pod cronjob exists in k8s or not. if it exists, check rolebinding microsegx-binding-scanner / microsegx-admin
	// rolebinding microsegx-binding-scanner is preferred in 5.2(+)
	// rolebinding microsegx-admin is majorly for backward compatibility
	if _, err := global.ORCH.GetResource(RscTypeCronJob, NvAdmSvcNamespace, "microsegx-updater-pod"); err == nil {
		...
		if errs, _ := VerifyNvRbacRoleBindings([]string{NvAdminRoleBinding}, existOnly, false); len(errs) > 0 {
			...
			if errs, k8sRbac403 := VerifyNvRbacRoleBindings([]string{NvScannerRoleBinding}, existOnly, true); len(errs) > 0 {
				if !k8sRbac403 {
					...
					// 403 error reading k8s rolebinding means clusterrolebinding "microsegx-binding-rbac" is incorrect
			...
			if errs, k8sRbac403 := VerifyNvRbacRoles([]string{NvScannerRole}, existOnly, true); len(errs) > 0 {
				if !k8sRbac403 {
					...
					// 403 error reading k8s role means clusterrolebinding "microsegx-binding-rbac" is incorrect
					clusterRoleBindingErrors = errs
				}
			}
		}
	...
	if errs, k8sRbac403 = VerifyNvRbacRoles(k8sClusterRoles, existOnly, true); !k8sRbac403 {
		...
		if errs, k8sRbac403 = VerifyNvRbacRoles(k8sRoles, existOnly, true); !k8sRbac403 {
			...
			if errs, k8sRbac403 = VerifyNvRbacRoleBindings(k8sClusterRoleBindings, existOnly, true); !k8sRbac403 {
				...
				// 403 error reading k8s clusterrolebinding means clusterrolebinding "microsegx-binding-rbac" is incorrect
				clusterRoleBindingErrors = errs
			}
		} else if len(errs) > 0 && len(clusterRoleBindingErrors) == 0 {
			// 403 error reading k8s role means clusterrolebinding "microsegx-binding-rbac" is incorrect
		...
		// 403 error reading k8s clusterrole means clusterrolebinding "microsegx-binding-rbac" is incorrect
		clusterRoleBindingErrors = errs
	}
