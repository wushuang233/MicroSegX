...
func TestRBAC(t *testing.T) {
	...
	rb := &k8sRoleBinding{
		...
		users: []k8sSubjectObjRef{
			{name: "mike", domain: "", subType: SUBJECT_USER},
		},
	}
	...
	rb = &k8sRoleBinding{
		...
		users: []k8sSubjectObjRef{
			{name: "mike", domain: "", subType: SUBJECT_USER},
			{name: "jane", domain: "", subType: SUBJECT_USER},
		},
	}
	if ev, old := d.updateResourceCache(rt, rb.uid, rb); ev != "" {
		d.cbResourceRoleBinding(rt, ev, rb, old)
...

	{
		...
		objCR1 := &rbacv1.ClusterRole{
			...
			Rules: []rbacv1.PolicyRule{
				{
					Verbs:     []string{"get"},
					APIGroups: []string{"read-only.microsegx.api.io"},
		...
		objCR2 := &rbacv1.ClusterRole{
			...
			Rules: []rbacv1.PolicyRule{
				...
				{
					Verbs:     []string{"*"},
					APIGroups: []string{"permission.microsegx.com"},
			...
			Rules: []rbacv1.PolicyRule{
				{
					Verbs:     []string{"get"},
					APIGroups: []string{"permission.microsegx.com"},
					Resources: []string{"microsegxvulnerability"}, // supported in global only
				},
				{
					Verbs:     []string{"get"},
					APIGroups: []string{"permission.microsegx.com"},
...

	{
		...
		objCR3 := &rbacv1.ClusterRole{
			...
			Rules: []rbacv1.PolicyRule{
				...
				{
					Verbs:     []string{"*"},
					APIGroups: []string{"microsegx.api.io"}, // not supported
					Resources: []string{"compliance", "cluster", "*"},
				},
				{
					Verbs:     []string{"*", "get"},
					APIGroups: []string{"permission.microsegx.com"},
					Resources: []string{"microsegxcluster"}, // not supported for namespaces
				},
				{
					Verbs:     []string{"*", "get"},
					APIGroups: []string{"permission.microsegx.com"},
...

	{
		...
		objCR31 := &rbacv1.ClusterRole{
			...
			Rules: []rbacv1.PolicyRule{
				...
				{
					Verbs:     []string{"*"},
					APIGroups: []string{"microsegx.api.io", "*"}, // unsupported apiGroup
					Resources: []string{"compliance"},
				},
				{
					Verbs:     []string{"*", "get"},
