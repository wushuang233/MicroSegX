...
In MicroSegX, proxy can be enabled/disabled in per-resource based.  For example, each registry can have different setting even when they connect to the same endpoints.  For these components that have their own http.Client, but would like to share a connection pool by using this package, use code similar to the snippet below:
