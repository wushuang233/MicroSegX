...
const PolicyIPRulesVersionID string = "MicroSegXPolicyVersion" // used for indicate policy version changed
const DlpRulesVersionID string = "MicroSegXDlpVersion"         // used for indicate dlp version changed
...
const CLUSScannerDBVersionID string = "MicroSegXCVEDBVersion" // used for indicate db version changed
...
const NsBoundaryKey string = "MicrosegxNamespaceBoundary"
...
const (
	...
	CLUSEvDEKSeedUnavailable         // dekSeed unavailable (most likely because of RBAC microsegx-binding-secret-controller)
	CLUSEvReEncryptWithDEK           // re-encrypt sensitive data in backup files with variant DEK
	CLUSEvEncryptionSecretSet        // microsegx-store-secret secret is set
)
...
type CLUSAdmissionCertCloaked struct { // a superset of CLUSAdmissionCert
	...
	KeyPath    string `json:"key_path"`          // obsolete, calc at runtime like "microsegx-svc-admission-webhook.{ns}.svc.key.pem"
	Key        []byte `json:"key"`               // not written to kv
	CertPath   string `json:"cert_path"`         // obsolete, calc at runtime like "microsegx-svc-admission-webhook.{ns}.svc.cert.pem"
...
type CLUSAdmCtrlState struct {
	Enable      bool   `json:"enable"`
	Uri         string `json:"uri"`           // for microsegx-validating-admission-webhook.microsegx.svc webhook
	NvStatusUri string `json:"nv_status_uri"` // for microsegx-validating-status-webhook.microsegx.svc webhook
}
...
type CLUSAdmissionState struct {
	...
	FailurePolicy  string                       `json:"failure_policy"`  // empty means "Ignore". it's only for microsegx-svc-admission-webhook
...
const (
	...
	CLUSJWTKey    = "microsegx-jwt-signing"
	CLUSTLSCert   = "microsegx"
)

func CLUSObjectCertKey(cn string) string {
	// ex: r object/cert/rootCA or object/cert/microsegx-svc-admission-webhook.microsegx.svc
