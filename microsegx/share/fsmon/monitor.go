...
func (w *FileWatch) StartWatch(id string, rootPid int, conf *FsmonConfig, capBlock, bMicrosegxSvc bool) {
	if !w.bEnable {
		return
	}
	...
	//// no access rules for microsegx and host
	...
	w.fanotifier.SetMode(rootPid, access, perm, capBlock, bMicrosegxSvc)

	w.addCoreFile(!bMicrosegxSvc, id, dirs, files)
