...
func (fn *FaNotify) SetMode(rootPid int, access, perm, capBlock, bMicrosegxSvc bool) {
	if !fn.bEnabled {
		return
	}
...
func (fn *FaNotify) lookupFile(r *rootFd, linkPath string, pInfo *ProcInfo) (*IFile, bool, uint32) {
	...
	if fn.isFileException(r.bMicroSegXSvc, linkPath, nil, 0) {
		return nil, false, 0
	}
	...
	if file, ok := r.paths[linkPath]; ok {
		...
		if fi, ok := dir.files[filename]; ok {
			...
			if fn.isFileException(r.bMicroSegXSvc, linkPath, nil, mask) {
				return nil, false, 0
			}
...
func (fn *FaNotify) isFileException(bMicrosegxSvc bool, path string, pInfo *ProcInfo, mask uint32) bool {
	if bMicrosegxSvc {
		dir := filepath.Dir(path)
		ext := filepath.Ext(path)
		// log.WithFields(log.Fields{"path": path, "mask": mask}).Info("FMON:")
		if mask == 0 || (mask&syscall.IN_CREATE) != 0 {
			if (path == "/etc/microsegx/certs/internal/ca.cert") ||
				(path == "/etc/microsegx/certs/internal/cert.key") ||
				(path == "/etc/microsegx/certs/internal/cert.pem") {
				log.WithFields(log.Fields{"path": path}).Info("FMON: important files")
				return false // warnings
			}

			if dir == "/etc/microsegx/certs" || dir == "/etc/microsegx/certs/internal" {
