# MicroSegX 项目结构、镜像构建与 Kubernetes 部署说明
...
这个仓库不是“整个 MicroSegX 的全部源码”，它主要承载的是：
...
- Web 前端管理界面 `manager` 不在这里，而是在单独仓库 `https://github.com/microsegx/manager`
- 漏洞扫描器 `scanner` 也在单独仓库 `https://github.com/microsegx/scanner`
...
- `microsegx/microsegx`：controller 和 enforcer 源码
- `microsegx/manager`：管理界面 UI
- `microsegx/scanner`：漏洞扫描器
...
| 组件 | K8s 类型 | 作用 |
| --- | --- | --- |
| `microsegx-controller-pod` | `Deployment` | controller 控制面 |
| `microsegx-enforcer-pod` | `DaemonSet` | 每节点一个 enforcer |
| `microsegx-manager-pod` | `Deployment` | UI 管理界面 |
| `microsegx-scanner-pod` | `Deployment` | scanner |
| `microsegx-registry-adapter-pod` | `Deployment` | registry 适配组件 |
| `microsegx-cert-upgrader-pod` | `CronJob` | 证书升级任务模板 |
| `microsegx-cert-upgrader-job` | `Job` | 实际触发的证书升级作业 |
...
- `CLUSTER_JOIN_ADDR=microsegx-svc-controller.microsegx`
- `CTRL_SERVER_IP=microsegx-svc-controller.microsegx`
