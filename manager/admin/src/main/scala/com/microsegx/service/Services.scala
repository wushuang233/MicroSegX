...
/**
 * Provides a hook to catch exceptions and rejections from routes, allowing custom responses to be
...
object Utils extends LazyLogging with Directives {
  ...
  /**
   * Wrap the web API response functions to add web server side response headers The function takes
   * boolean parameters to verify and determine which headers are needed.
   *
   * @param isStaticResource
   *   Boolean: True if requested resource is static resource. e.g.: html, css, js files
   * @param isJs
   *   Boolean: True if requested resource is javascript files
   * @return
   *   Directive for wrapping API response functions
   */
