package com.microsegx.core

import com.microsegx.core.CommonSettings.{ newCert, newKey, newMgrCert, newMgrKey }
...
trait MySslConfiguration extends LazyLogging {
  ...
  private def generateSelfSignedCertificate(keyPair: KeyPair): X509Certificate = {
    // Certificate details
    val issuer    = new X500Name("CN=microsegx, O=MicroSegX, L=San Jose, ST=California, C=USA")
    val subject   = new X500Name("CN=microsegx")
    ...
    sanBuilder.addName(new GeneralName(GeneralName.dNSName, "microsegx"))
