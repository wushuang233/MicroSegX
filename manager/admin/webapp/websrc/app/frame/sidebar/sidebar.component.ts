...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  customFooterStyle;
  customSideBarStyle;
  isPreviewMode = environment.previewMode;
  ...
  ngOnInit() {
    ...
    if (this.isPreviewMode) {
      this.menuItems = this.limitPreviewMenu(this.menuItems);
    }
    if (
  ...
  private limitPreviewMenu(menu: Array<any>): Array<any> {
    const enabledLinks = new Set([
      '/dashboard',
      '/workloads',
      '/regScan',
      '/scan',
    ...
    return menu
      .map(item => {
        if (item.link) {
          return enabledLinks.has(item.link) ? item : null;
        }
        if (item.submenu) {
          const submenu = item.submenu.filter(subItem =>
            enabledLinks.has(subItem.link)
          );
