...
export class ContainersGridComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.gridOptions = {
      ...
      rowClassRules: {
        'workload-row--child': params => !!params.data?.parent_id,
      },
      components: {
  ...
  private resizeColumns(): void {
    ...
    const gridWidth = document
      .getElementById('containers-grid')
      ?.getBoundingClientRect().width;
    if (gridWidth && gridWidth > 0) {
      if (gridWidth >= this.minimumComfortGridWidth) {
        ...
        this.gridApi.applyColumnState({
          state: [
            ...
            { colId: 'security.scan_summary.medium', width: 108 },
            { colId: 'security.scan_summary.scanned_at', width: 184 },
          ],
        });
      }
    }
