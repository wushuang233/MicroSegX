...
export class ContainersGridNameCellComponent implements ICellRendererAngularComp {
  ...
  agInit(params: ICellRendererParams): void {
    ...
    this.service = params.data.brief.service || params.data.brief.name || '-';
    this.domain = params.data.brief.domain || '-';
    this.host = params.data.brief.host_name || '-';
    this.isChild = !!params.data.parent_id;
    this.isParent =
      !params.data.parent_id &&
      params.data.child_ids &&
      params.data.child_ids.length > 0;
    this.childCount = params.data.child_ids?.length || 0;
  }
