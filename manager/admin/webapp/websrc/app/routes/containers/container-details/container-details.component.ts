...
export class ContainerDetailsComponent implements OnInit, OnDestroy {
  ...
  get containerStateLabel(): string {
    return this.utils.getI18Name(this.container?.brief?.state || 'unmanaged');
  }
  get scanStatusLabel(): string {
    return this.utils.getI18Name(
      this.container?.security?.scan_summary?.status || 'finished'
    );
  }
  get policyModeLabel(): string {
    return this.utils.getI18Name(
      this.container?.security?.policy_mode || 'discover'
    );
  }
  get profileModeLabel(): string {
    return this.utils.getI18Name(
      this.container?.security?.profile_mode || 'discover'
    );
  }
  get applicationSummary(): string {
    const applications = this.container?.rt_attributes?.applications || [];
    return applications.length ? applications.join(' · ') : '-';
  }
  get portCount(): number {
    return this.container?.rt_attributes?.ports?.length || 0;
  }
  get interfaceCount(): number {
    return Object.keys(this.container?.rt_attributes?.interfaces || {}).length;
  }
  get highCount(): number {
    return this.container?.security?.scan_summary?.high || 0;
  }
  get mediumCount(): number {
    return this.container?.security?.scan_summary?.medium || 0;
  }
  get quarantineReason(): string {
    return this.container?.security?.quarantine_reason || '';
  }
